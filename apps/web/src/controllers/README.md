...
| File | Purpose |
|------|---------|
| `attachmentController.ts` | Attachment upload/management + incremental sync enqueue + BinaryRef hash metadata updates (creates missing `BinaryRef` entries when needed, canonicalizes attachment paths before CRDT metadata writes, and prefers active CRDT workspace id for upload queueing) |
