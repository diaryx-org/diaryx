...
| `onboardingController.ts` | Onboarding orchestration (E2E bypass, starter workspace seeding, iOS first-run bootstrap, default workspace auto-creation, folder workspace create/open, bundle application, welcome screen callback orchestration). Folder targets register a local workspace first, then initialize the backend against that selected directory/handle before seeding starter content. Legacy provider restore helpers remain for old sync tests/cleanup work, but the active welcome UI no longer exposes remote workspace restore or provider-backed creation. Pure .ts with dependency injection for testability. |
