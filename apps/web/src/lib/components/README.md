...
| `DocumentAudiencePill.svelte` | Entry-level audience tag pill used by properties/sidebar and namespace widgets; normalizes legacy scalar `audience: tag` frontmatter and malformed values before rendering so imported folders cannot crash the panel. |
...
| `PluginStatusItems.svelte` | Legacy footer status-bar renderer for plugin-contributed status items. It is retained for compatibility but is not used for built-in workspace sync controls. |
| `PluginIframe.svelte` | Sandboxed plugin iframe host with direct `get_component_html` loading when the runtime exposes it, command bridge fallback for older/plugin-command-only guests, managed-context injection for `diaryx.ai`, and response-shape normalization for plugin HTML payloads. Guests may opt into content-sizing by posting `{ type: "content-size", height }` — the host then drops `flex-1` and sizes the iframe to the reported height so the surrounding column (not the iframe) handles scrolling. |
