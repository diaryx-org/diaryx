...
function readArtifact(
  ...
  // A present-but-incomplete artifact (no url, or a non-positive size) describes a
  // bundle with no published download — e.g. a locally-authored/unpublished bundle
  // imported from a file. Treat it as "no artifact" rather than rejecting the whole
  // bundle; downstream consumers already handle a null artifact (`item.artifact?.…`).
  ...
  if (!hasUrl || !hasSize) {
    if (hasUrl || hasSize) {
      console.warn("Bundle artifact incomplete; treating as unpublished (no artifact):", artifactRaw);
    }
    return null;
  }
