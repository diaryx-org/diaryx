...
function readArtifact(
  ...
  // an item with no published download — e.g. a locally-authored/unpublished
  // starter workspace imported from a file. Treat it as "no artifact" rather than
  // rejecting the whole entry; downstream consumers already handle a null artifact.
  ...
  if (!hasUrl || !hasSize) {
    if (hasUrl || hasSize) {
      console.warn("Starter workspace artifact incomplete; treating as unpublished (no artifact):", artifactRaw);
    }
    return null;
  }
