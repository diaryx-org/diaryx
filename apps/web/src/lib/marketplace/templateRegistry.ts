...
function readArtifact(
  ...
  // template imported from a file. Treat it as "no artifact" rather than rejecting
  ...
  if (!hasUrl || !hasSize) {
    if (hasUrl || hasSize) {
      console.warn("Template artifact incomplete; treating as unpublished (no artifact):", artifactRaw);
    }
    return null;
  }
