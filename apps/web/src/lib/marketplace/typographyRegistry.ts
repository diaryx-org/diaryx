...
function readArtifact(
  ...
  // typography imported from a file. Treat it as "no artifact" rather than rejecting
  // the whole entry; downstream consumers already handle a null artifact.
  ...
  if (!hasUrl || !hasSize) {
    if (hasUrl || hasSize) {
      console.warn("Typography artifact incomplete; treating as unpublished (no artifact):", artifactRaw);
    }
    return null;
  }
