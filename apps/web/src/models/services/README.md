...
| File | Purpose |
|------|---------|
| `attachmentService.ts` | Attachment blob URL transform/reverse. Resolves plain attachment paths directly first (then falls back to link-parser normalization via `canonicalizeLink`/`formatLink`) to reduce backend command contention during file switching; uses low-concurrency queued lazy resolution in editor NodeViews. Falls back to server streaming via `getServerAttachmentUrl()` when local data is missing. Verifies local file hash against CRDT metadata and re-fetches from server on mismatch (attachment updated on another device). |
