...
fn report_and_fix_validation(
    ...
    if !result.errors.is_empty() {
        ...
        for err in &result.errors {
            let fix_result = if fix {
                Some(block_on(fixer.fix_error(err)))
            } else {
                None
            };
            match fix_result {
                Some(fr) if fr.success => {
                    println!("  ✓ Fixed: {}", fr.message);
                    fixed_count += 1;
                }
                Some(fr) => {
                    println!(
                ...
                None => {
                    println!("  ✗ {}: {}", err.description(), err.detail());
                }
            }
        }
    }
    ...
    if !result.warnings.is_empty() {
        ...
        for warn in &result.warnings {
            let fix_result = if fix && warn.can_auto_fix() {
                block_on(fixer.fix_warning(warn))
            } else {
                None
            };
            match fix_result {
                ...
                None => {
                    println!("  ⚠ {}: {}", warn.description(), warn.detail());
                }
            }
        }
    }
