-- Initial schema for Diaryx D1 database.
-- Matches the native sync server schema as of v1.4.0.
...
CREATE INDEX idx_users_apple_tx
...
CREATE INDEX idx_magic_tokens_code ON magic_tokens(code);
...
CREATE INDEX idx_user_ai_usage_monthly_user ON user_ai_usage_monthly(user_id);
...
CREATE INDEX idx_namespace_objects_ns ON namespace_objects(namespace_id);
CREATE INDEX idx_namespace_objects_audience ON namespace_objects(namespace_id, audience);

-- Namespace audiences
CREATE TABLE namespace_audiences (
    namespace_id TEXT NOT NULL REFERENCES namespaces(id) ON DELETE CASCADE,
...
CREATE INDEX idx_namespace_audiences_ns ON namespace_audiences(namespace_id);
...
CREATE INDEX idx_custom_domains_ns ON custom_domains(namespace_id);
...
CREATE INDEX idx_usage_events_user ON usage_events(user_id, event_type, recorded_at);
CREATE INDEX idx_usage_events_recorded ON usage_events(recorded_at);

-- Namespace sessions
CREATE TABLE namespace_sessions (
...
CREATE INDEX idx_namespace_sessions_owner ON namespace_sessions(owner_user_id);
CREATE INDEX idx_namespace_sessions_ns ON namespace_sessions(namespace_id);
