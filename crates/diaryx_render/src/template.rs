...
//!
//! Moved here from the `diaryx_templating_extism` plugin so the same render-time
//! templating can run server-side (ARK Layer 3) as well as in the plugin. Pure
//! Rust — no Extism, no host functions. Gated behind the `templating` feature.
//!
