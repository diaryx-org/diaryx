...
mod tests {
    ...
    fn test_sync_between_protocols() {
        let storage1: Arc<dyn crate::CrdtStorage> = Arc::new(MemoryStorage::new());
        let storage2: Arc<dyn crate::CrdtStorage> = Arc::new(MemoryStorage::new());
        ...
        let metadata = crate::FileMetadata::new(Some("Test File".to_string()));
        protocol1
            .workspace_mut()
    ...
    fn test_update_message() {
        let storage: Arc<dyn crate::CrdtStorage> = Arc::new(MemoryStorage::new());
        let workspace = WorkspaceCrdt::new(storage);
        let mut protocol = SyncProtocol::new(workspace);

        // Make a change
        let metadata = crate::FileMetadata::new(Some("New File".to_string()));
        protocol
            .workspace_mut()
